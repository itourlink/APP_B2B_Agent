...
export const GenericFilter = ({
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div
              ...
              className={`flex items-center w-12 h-[25px] rounded-full cursor-pointer ${
                values[item.key] ? "bg-[#2566b0]" : "bg-gray-300"
              }`}
            >
              <div
                className={`w-5 h-5 bg-white rounded-full transition ${
                  values[item.key] ? "translate-x-[25px]" : "translate-x-[2px]"
                }`}
              />
            </div>
          </div>
      ...
      case "guest":
        return (
          <div ref={guestRef} className="relative">
            <button
              onClick={() => setActive(active === "guest" ? null : "guest")}
      ...
      case "date":
        return (
          <div ref={dateOneRef} className="relative">
            <button
              onClick={() => setActive(active === "dateOne" ? null : "dateOne")}
              className="cursor-pointer"
            >
              <div className="text-sm font-semibold">
                {item.label || t("date")}
  ...
  return (
    <div className="w-full flex justify-center">
      <div
        className="bg-white rounded-xl shadow-2xl p-6 flex gap-5 items-center"
        onKeyDownCapture={(e) => {
      ...
      >
        {filters
          .filter((item) => {
            if (item.type === "tourType" && values.isTourSeries) {
              return false;
            }
