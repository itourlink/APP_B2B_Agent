...
const AddManual = ({
  ...
  const onSubmit = methods.handleSubmit(async (values) => {
    ...
    const payload = {
      ...
      strTourCustomizedDayGUID: strTourCustomizedDayGUID || null,

      strLocationCode:
        values.cityCode || values.regionCode || values.countryCode || null,

      intPerPaxID: values.unitId ? Number(values.unitId) : null,

      strServiceName: values.serviceName.trim() || null,

      strSupplierName: values.supplierName.trim() || null,

      intCurrencyID: values.currencyId ? Number(values.currencyId) : null,

      dblPrice: values.price ? Number(values.price) : 0,

      dblSglSupPrice: isOvernight
        ? Number(values.sglSupPrice || 0)
        : 0,

      dblTplRedPrice: isOvernight
        ? Number(values.tplRedPrice || 0)
        : 0,
      ...
      strRemark: values.remark?.trim() || null,
    };
            ...
            <div className="col-span-3">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <Field.SearchSelect
                  ...
                  disabled={!regionCode}
                  placeholder={
                    isCityLoading ? t("loadingCities") : t("selectCityOption")
                  }
                />
              </div>
            </div>
          ...
          <div className="mt-6 rounded-3xl border border-gray-300 p-5">
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
              <div>
                ...
                >
                  <option value="">
                    {isCurrencyLoading ? "Đang tải..." : "Chọn tiền tệ"}
                  </option>
