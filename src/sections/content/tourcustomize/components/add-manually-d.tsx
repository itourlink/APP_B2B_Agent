...
ORDER BY MC04_CityName`
  ...
  if (!values.supplierName.trim()) {
   ...
   const payload = {
    ...
    strLocationCode:values.cityCode || values.regionCode || values.countryCode,
    intPerPaxID: Number(values.unitId),
    strServiceName: values.serviceName.trim(),
    strSupplierName: values.supplierName.trim(),

    dblPrice: Number(values.price || 0),
    dblSglSupPrice: isOvernight ? Number(values.sglSupPrice || 0) : 0,
    dblTplRedPrice: isOvernight ? Number(values.tplRedPrice || 0) : 0,

    isForSleep: isOvernight,

    intCurrencyID: Number(values.currencyId),

    strRemark: values.remark?.trim() || "",

    isPriceByRoom: isOvernight ? isPriceByRoom : false,
  };
      ...
      <div className="w-full font-sans">
        <div className="p-1">
          <div className="grid grid-cols-4 gap-x-6 gap-y-5">
            ...
            <div className="col-span-1">
              <label className="mb-1 block text-sm text-gray-700">
                Ten nha cung cap <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                ...
                <div>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      isOvernight ? "bg-[#3b82f6]" : "bg-gray-300"
                    }`}
                  >
                    <span
                ...
                {isOvernight && (
                  <div>
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                        isPriceByRoom ? "bg-[#3b82f6]" : "bg-gray-300"
                      }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          isPriceByRoom ? "translate-x-5" : "translate-x-0"
                        }`}
                      />
                    </button>
                  </div>
        ...
        <div className="flex justify-end pt-6">
          <button
            type="submit"
            disabled={isSaving}
            className="rounded bg-[#004a99] px-10 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-[#003875] focus:outline-none"
          >
            {isSaving ? "Đang lưu..." : "Luu"}
          </button>
        </div>
      </div>
    </Form>
