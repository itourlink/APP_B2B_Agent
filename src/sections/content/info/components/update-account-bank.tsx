...
import { useUpdCompanyBankAccount } from "@/hooks/actions/useUser";
...
const UpdateAccountBank = ({ item, onClose }: Props) => {
    const queryClient = useQueryClient();
    const { showToast } = useToastStore();
    const user = useUserStore((state) => state.user);
    const { mutate: useUpdCompanyBankAccountApi, isPending: isLoading } = useMutation({
        mutationFn: useUpdCompanyBankAccount,
    });
    const defaultValues: SchemaType = {
        displayName: `${item?.strNameDisplay}`,
        accountInfo: `${item?.strCompanyBankAccountInfo}`,
        accountName: `${item?.strCompanyBankAccountName}`,
        accountNumber: `${item?.strCompanyBankAccountCode}`,
        swiftCode: `${item?.strSwiftCode}`,
        branch: `${item?.strBankAddress}`,
    };
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        useUpdCompanyBankAccountApi(payload, {
            onSuccess: () => {
            ...
            onError: () => {
                showToast("error", "Cập nhật tài khoản thất bại");
            },
        });
    });
