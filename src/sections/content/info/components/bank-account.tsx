...
import {
  ...
  useGetFilterCompanyBankAccount,
...
const BankAccount = () => {
  ...
  const { mutate: useGetFilterCompanyBankAccountApi, isPending: isSearchLoading } =
    useMutation({
  ...
  const colDefs: ColumnDef<ICompanyBankAccount>[] = [
...

    {
      ...
      headerName: t("accountNumberSwift"),
      render: (_, row) => (
        <div className="flex flex-col">
          <span className="text-[#004b91] font-mono font-bold">
            {row.strCompanyBankAccountCode}
          </span>
          <span className="text-[10px] text-orange-600 font-semibold bg-orange-50 px-1.5 py-0.5 rounded w-fit mt-1">
            {t("swiftCode")}: {row.strSwiftCode}
          </span>
        </div>
      ),
    },
      ...
      <div className="mt-4">
          ...
          search={[
            {
              ...
              placeHoder: t("bankAccountName"),
            },
            {
              keySearch: "idAccount",
              value: filters.idAccount,
              placeHoder: t("accountCode"),
            },
            {
              keySearch: "idSwift",
              value: filters.idSwift,
              placeHoder: t("swiftCode"),
            },
            {
              keySearch: "addRess",
              value: filters.addRess,
              placeHoder: t("address"),
            },
          ]}
        />
        ...
        {open.delete && (
          <PanelPopup
            ...
            title={t("confirmDeleteBankAccount")}
          >
            <DeleteAccountBank
              item={item}
              onClose={() => setOpen((prev) => ({ ...prev, delete: false }))}
