...
import PaymentTableBooking from "./payment-booking-tour/components/payment-table-booking";
import PaymentVouBank from "./payment-booking-tour/components/payment-vou-bank-booking";
import PaymentFormBooking from "./payment-booking-tour/components/payment-form-booking";
...
const PaymentBookingView: React.FC = () => {
  ...
  // ⚠️ FIX: KHÔNG return JSX sớm ở đây nữa (trước đây nằm giữa các hook
  // -> vi phạm Rules of Hooks, khiến số lượng hook thay đổi giữa các lần
  // render khi isExpired chuyển true trong lúc handleBooking() đang chạy
  // dở, làm các mutation (listAGTMS/detailAGTMS) bị stale/huỷ ngang).
  // Việc quyết định hiển thị gì được dời xuống phần return cuối cùng.
  ...
  const syncAgentHostServiceToTMS = async (
    ...
    try {
      ...
      console.log("2. GetListAgentHostServiceTransToTMS SUCCESS:", listResult);
    } catch (listError) {
      console.error("2. GetListAgentHostServiceTransToTMS ERROR:", listError);
    }
    ...
    try {
      ...
      console.error("4. GetDetailBookingServiceTransToTMS ERROR:", detailError);
      throw detailError;
    }
  };
  ...
  const handleBooking = async () => {
    ...
    if (isShowTravellerForm) {
      const travellerSchema = z.object({
        intSaluteID: z
          .string()
          .min(1, t("pleaseSelectTitle") || "Vui lòng chọn danh xưng"),

        strPassengerFirstName: z
          .string()
          .trim()
          .min(1, t("pleaseEnterFirstName") || "Vui lòng nhập tên"),

        strPassengerLastName: z
          .string()
          .trim()
          .min(1, t("pleaseEnterLastName") || "Vui lòng nhập họ và tên đệm"),

        strCountryGUID: z
          .string()
          .min(1, t("pleaseSelectCountry") || "Vui lòng chọn quốc tịch"),

        dtmPassengerBirthday: z
          .string()
          .min(1, t("pleaseEnterDateOfBirth") || "Vui lòng chọn ngày sinh"),
        ...
        strPassengerPhone: z
          .string()
          .trim()
          .min(1, t("pleaseEnterPhoneNumber") || "Vui lòng nhập số điện thoại"),
      });
    ...
    try {
      ...
      console.log("2. AddBookingForTour SUCCESS:", res);

      showToast("success", t("bookingSuccess"));

      // ======================================================
      // 7. GET DATA FROM BOOKING RESPONSE
      // ======================================================

      const bookingResult = res?.[1]?.[0];

      console.log("Booking result:", bookingResult);

      const intBK = bookingResult?.intStatusBk;

      const serviceGUID = bookingResult?.strListAgentHostServiceItemGUID;
      ...
      // ======================================================
      // PHẢI CÓ SERVICE GUID
      // ======================================================

      if (!serviceGUID) {
        console.error("Không có strListAgentHostServiceItemGUID", res);
      ...
      try {
        console.log("3. CALL GetEmailSendAgentHostByAgentBook");
        ...
        console.log("4. EMAIL DATA:", emailData);

        if (emailData) {
          console.log("5. CALL GetSendEmail");
          ...
          console.log("6. GetSendEmail SUCCESS");
        }
      } catch (emailError) {
        console.error("SEND EMAIL ERROR:", emailError);

        // Email lỗi không chặn TMS
      }
      ...
      try {
        console.log("7. START SYNC TMS");

        await syncAgentHostServiceToTMS(companyGUID, serviceGUID);

        console.log("8. SYNC TMS COMPLETE");
      } catch (tmsError) {
      ...
      console.log("9. REDIRECT:", activeTab);
  ...
  // ⚠️ FIX: quyết định render "trang trắng khi hết hạn" nằm ở đây,
  // SAU khi tất cả hook đã được gọi đầy đủ và theo đúng thứ tự mỗi lần render.
  if (isExpired) {
